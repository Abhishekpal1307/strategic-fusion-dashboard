-- Intelligence nodes table
create table if not exists public.intel_nodes (
...
create index if not exists idx_intel_nodes_user on public.intel_nodes(user_id);
create index if not exists idx_intel_nodes_created on public.intel_nodes(created_at desc);

-- Storage bucket for IMINT images
